...
*Why the rule leans toward false positives:* "the stop sign" (three words) is treated as an opt-out; "I stopped by the lot" and "Can you stop by Tuesday? I have questions about the truck" are not. A false positive costs one customer that Mick can reactivate with a tap after a real conversation. A false negative means sending to someone who asked us to stop, which is a legal exposure. We chose the cheap mistake. Known miss the rule does not catch: "please opt *me* out" (the words are split) — Mick marks those by hand from the timeline, which he reads anyway because every reply lands there.
