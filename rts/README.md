...
**Interactive:** [`game/index.html`](game/index.html) — **▶ the playable skirmish**: pick your faction, the CPU's faction, difficulty, and one of **three maps**; real units/economy/combat with A\* pathfinding, energy shields, stealth/detection, **capturable relay towers** (detection objectives), **finite rich ore**, camera zoom + follow, minimap, control groups, and sound · [`lab/index.html`](lab/index.html) — balance calculator + budget validator · [`prototype/index.html`](prototype/index.html) — visual battle sandbox (composition sim) · [`maps/twin-ridge.html`](maps/twin-ridge.html) — map viewer · [`style/index.html`](style/index.html) — cel-shaded style guide. All single-file, offline.
...
- ~~**Competitive maps**~~ **3-map ranked pool (§38)** — **Twin Ridge** (moderate choke + multi-route), **Open Steppe** (open vehicle/expansion), **Scrapline** (tight canyon/infantry) — all pass **19/19** map tests via [`map-check.mjs`](scripts/map-check.mjs), now including a nav-grid **reachability** check (BFS over the exact in-game rasterization). Selectable in-game.
- **Live-engine balance (telemetry-driven, §49-clean)** — the first batch showed Covenant 100% / Array 6%; a 198-match commit-policy experiment **falsified** the tempo hypothesis and pinned the real cause: the CPU picker was blind to range/splash (it walked into Interceptor Gyros and never bought AA). Fixed with the P7 commit rule (+stalemate breaker) and a two-sided trade-ratio picker — **no unit stats touched**. Post-fix: Covenant 69%, Array 75%, Covenant–Array dead even. Residual: Directorate weak in CPU-vs-CPU brawls (its repair/siege identity isn't in the v1 sim) — flagged as the next calibration item. [design/21](design/21-engine-structure-review.md)
